...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} -1 random no grupo🙄🤙🏻`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ calma porra, tô fazendo... ⌛',
				success: 'processo feito 📌',
				error: {
					stick: '❌ Falha, ocorreu um erro ao converter a imagem em um adesivo, deve ser pq vc é gay ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '⚠️ Este comando só pode ser usado pelo grupo proprietário! ⚠️',
			...
			switch(command) {
             ...
             case 'lacoste':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://static.biologianet.com/2020/05/jacare-do-papo-amarelo.jpg`)
				...
				case 'nsfwloli':
					...
					break*/
				case 'hilih':
					if (args.length < 1) return reply('Onde está o texto porra?')
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/hilih?teks=${body.slice(7)}`, {method: 'get'})
					reply(anu.result)
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Onde está a url porra?')
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('Qual é o tipo man?')
					if (!tipelist.includes(args[0])) return reply('Tipe desktop|tablet|mobile')
					if (args.length < 2) return reply('Cadê a url porra?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Onde está o texto man?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'tagall':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('Quem é vc man?')
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Esse maluco aqui @${_.split('@')[0]}\nAcabou de ser promovido, ent respeitem, ok?😎`
						}
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Esse maluco aqui @${_.split('@')[0]}\nAcabou de perder o adm KKKKK pressionem F aí chat 😎`
						}
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país porra!')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('❗Falha ao adicionar o gay no grupo❗')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Acabou de levar ban @${_.split('@')[0]}\nOtário KKKKKKK só n vai chorar hein😎`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
				...
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista de adm no grupo: *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda ao adesivo hum ❌')
					reply(mess.wait)
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadê o texto fdp?')
					teks = body.slice(5)
